...
export async function checkMembership(accessToken) {
  ...
  // A user can support multiple campaigns; this token is scoped to YOURS, so memberships here are for you.
  const m = members[0]?.attributes;
  if (!m || m.patron_status !== 'active_patron') return { status: 'none', name };
  ...
  return { status: charged ? 'paid' : 'trial', name, cents };
}
