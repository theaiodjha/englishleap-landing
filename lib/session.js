...
export function patreonAuthUrl(state) {
  ...
  // identity = who they are; identity[email] = their email (shown back to help them
  // spot a wrong account); identity.memberships = their membership to YOUR campaign
...
// Which tier a session is on, as one word the UI can label. Derived from `cents`
// (the price they actually pay) rather than s.tier, which is 'fluency' for ANY paid
// pledge and so calls a $1 Transcript backer a Fluency member. A legacy session with
// no `cents` predates tiering and counts as full — same rule as the Fluency gate.
export function planOf(s) {
  if (!s) return 'none';
  if (s.cents === undefined || s.cents === null) return 'fluency';
  if ((Number(s.cents) || 0) >= 200) return 'fluency';
  if (s.access === 'trial') return 'trial';
  return 'transcript';
}

// Re-confirm a session's membership with Patreon at most every RECHECK_HOURS — and
// immediately for a legacy session issued before tiering (no `cents`), so it gets one.
// Returns the refreshed session, or null when the membership is now inactive (the cookie
// is cleared; the caller decides what to say). Sessions without a refresh token — member
// codes from /api/unlock — are returned untouched. Patreon unreachable → the cached
// session is honoured, so an outage never locks out a paying member.
export async function revalidateSession(res, s) {
  if (!s) return s;
  ...
  try {
    ...
    const next = { ...s, at, rt, atexp, access: mem.status, cents: mem.cents,
      tier: mem.status === 'paid' ? 'fluency' : 'trial', recheck: Date.now() + RECHECK_HOURS * 3600e3 };
...
export async function checkMembership(accessToken) {
  ...
  u.searchParams.set('fields[user]', 'full_name,email');
  ...
  const email = j?.data?.attributes?.email || null; // shown back to spot a wrong account
  ...
  if (!m || m.patron_status !== 'active_patron') {
    ...
    return { status: 'none', name, email, id };
  }
  ...
  return { status: charged ? 'paid' : 'trial', name, email, cents, id };
}
