...
// True when the member has used none of this episode's words yet: every one is new,
// so none of them is "the one you are behind on".
export function allNew(words, counts) {
  const ws = Array.isArray(words) ? words : [];
  return ws.length > 0 && ws.every((w) => ((counts || {})[key(w)] || 0) === 0);
}
...
//
// Returns NOTHING when they have used none of them. Singling out two of six identical
// zeroes only looks personal: the words are indistinguishable, and telling someone they
// "have not used these two yet" is arbitrary dressed up as a recommendation. Spaced
// repetition needs something to fall behind on; use allNew() for the first visit.
export function focusFor(words, counts, max = FOCUS_MAX) {
  const scored = (Array.isArray(words) ? words : [])
    .map((w) => ({ w, n: (counts || {})[key(w)] || 0 }));
  if (!scored.some((x) => x.n > 0)) return [];        // no signal yet
  return scored
...
/* Why these two words, said truthfully.
   focusFor() mixes two different situations: a word never spoken (count 0) and a word
   spoken once or twice but not yet owned. Calling both "you have not used it yet" is
   false for the second, and a member who used the word a minute ago will notice. They
   also want different advice, so they get different sentences. */
function focusWhy(focus, counts) {
  const n = (w) => ((counts || {})[key(w)] || 0);
  const unused = focus.filter((w) => n(w) === 0);
  const rusty = focus.filter((w) => n(w) > 0);          // used, but not owned yet
  if (unused.length > 1) return `You have not used “${unused[0]}” or “${unused[1]}” out loud yet.`;
  if (unused.length === 1 && rusty.length) {
    return `You have not used “${unused[0]}” out loud yet — and “${rusty[0]}” could use another go.`;
  }
  if (unused.length === 1) return `You have not used “${unused[0]}” out loud yet.`;
  if (rusty.length > 1) return `“${rusty[0]}” and “${rusty[1]}” need another outing or two to stick.`;
  if (rusty.length === 1) return `“${rusty[0]}” needs another outing or two to stick.`;
  return 'Reach for this week’s phrases while they are fresh.';
}
...
export function nextAction({ episodes, counts, games, sessions, types } = {}) {
  ...
  const curFresh = allNew(cur.words, counts);
  if (curFocus.length || curFresh) {
    return {
      kind: 'speak',
      episodeId: cur.id, episode: cur.n, title: cur.title,
      words: curFocus,                       // empty when the whole episode is new
      fresh: curFresh,
      minutes: MIN_PER_SESSION,
      why: curFresh
        ? `${cur.words.length} new phrases this week — see how many you can work in.`
        : focusWhy(curFocus, counts),
      cta: 'Practise speaking',
      href: `/out-loud.html?ep=${encodeURIComponent(cur.id)}`,
    };
  }
  ...
  for (const id of practised) {
    ...
    // Having practised an episode without using ANY of its phrases is itself a reason to
    // come back — focusFor() sees no signal there, so check allNew() too.
    const fresh = allNew(ep.words, counts);
    if (!focus.length && !fresh) continue;
    return {
      kind: 'revise',
      ...
      minutes: MIN_PER_SESSION,
      why: focus.length
        ? `“${focus[0]}” from EP${ep.n} has not settled yet — come back to it.`
        : `You have spoken about EP${ep.n}, but none of its phrases have landed yet.`,
      cta: 'Practise again',
      href: `/out-loud.html?ep=${encodeURIComponent(ep.id)}`,
    };
  }
