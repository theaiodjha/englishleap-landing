...
import { EPISODE_TITLES } from "./episode-titles.js";
...
// ---- catalogue views shared by Out Loud and the coach ----
// clue-room is the free-tier game, so it carries EVERY episode, and its six clue words
// are the vocabulary that episode teaches. One source of truth for both.
export async function getEpisodes() {
  ...
  return eps
    .map((e) => ({
      id: e.id,
      n: Number(String(e.ep || e.id).replace(/\D/g, "")) || 0,
      title: EPISODE_TITLES[e.id] || e.title,
      words: ((e.content && e.content.clues) || []).map((c) => c.word).filter(Boolean),
      /* Each word's validated palette colour and its emoji, kept rather than thrown away:
         they are what the word wears in every game, so anything showing these six words can
         show them with the identity members already recognise. */
      wordMeta: ((e.content && e.content.clues) || [])
        .filter((c) => c.word)
        .map((c) => ({ w: c.word, c: c.color || null, e: c.emoji || null })),
      current: !!e.current,
    }))
    .filter((e) => e.words.length);
}

// The game types themselves, for "you have not tried this one yet".
export async function getGameTypes() {
  const arcade = await getArcade();
  return arcade.map((g) => ({ type: g.type, name: g.name, access: g.access || "fluency" }));
}
