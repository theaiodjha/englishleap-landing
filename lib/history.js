...
/* What a member has chosen to tell the coach about how they speak. A SETTING, so no TTL:
   practice history ages out, a person's voice does not. Kept short deliberately — it has to
   fit in a prompt, and one clear line is more useful to the coach than a paragraph. */
const noteKey = (uid) => `uil:note:${uid}`;
export const NOTE_MAX = 200;

/* Free text from a member that ends up inside a model prompt, so it is flattened to a
   single line and stripped of control characters before it is ever stored. That is not
   about trust — it is that a newline in the middle of a prompt is how a sentence starts
...
export async function getNote(uid) {
  if (!ready() || !uid) return '';
  try { return cleanNote(await read(`get/${encodeURIComponent(noteKey(uid))}`)); }
  catch { return ''; }
}

// Empty clears it — a member takes back what they said by deleting the words, which is
// what the control looks like it does.
export async function setNote(uid, text) {
  if (!ready() || !uid) return '';
  const clean = cleanNote(text);
  try {
    if (!clean) await pipeline([['DEL', noteKey(uid)]]);
    else await pipeline([['SET', noteKey(uid), clean]]);
    return clean;
  } catch { return clean; }
}
